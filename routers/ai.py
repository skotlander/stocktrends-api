...
def ai_context():
    ...
    return {
        ...
        "usage_guidance": [
            ...
            "For x402, the HTTP 402 stocktrends_preview is the final pre-payment surface: inspect it to confirm purpose, inputs, response shape, related endpoints, pricing_rule_id, cost, and rails before paying. It is not the mechanism for discovering endpoint parameters — use /v1/ai/tools or /v1/openapi.json for input schemas, and make the request serviceable before paying. Receiving the challenge does not require a serviceable request: an eligible recognized fixed-price resource answers a bare unpaid probe with it, while availability-gated and parameterized resources resolve availability or path identity first.",
...
def ai_tools():
    ...
    return {
        ...
        "agent_onboarding_notes": [
            ...
            "For x402, construct a serviceable request from /.well-known/x402 and /v1/openapi.json before paying; an unpaid probe of an eligible recognized fixed-price resource returns its 402 payment contract whether or not the request is complete, while availability-gated and parameterized resources resolve availability or path identity first. Each resource publishes its own challenge_lifecycle. The 402 stocktrends_preview is a final confirmation, not the schema-discovery mechanism.",
            "Once a payment proof is presented, deterministically invalid input is rejected before any payment challenge or settlement: a malformed symbol_exchange, a missing required parameter combination, an unsupported sort/exchange/trend value, or a semantically incomplete body returns a 400 or 422 input error and nothing is verified or settled. An unpaid probe of an eligible recognized fixed-price resource is answered with the 402 challenge instead, so its payment and input contract is readable at the canonical resource URL; availability-gated and parameterized resources are documented exceptions. Pricing context headers remain on the input error, so the price is still discoverable once the request is corrected.",
        ],
        "tools": tools,
        "workflows": workflows,
        ...
        "notes": [
            ...
            "For x402, inspect the 402 stocktrends_preview before paying; it is the final pre-payment surface, not an input-schema reference — use /v1/ai/tools or /v1/openapi.json to make the request serviceable before paying, which is not a precondition for receiving the challenge.",
