...
HISTORY_PATH = "/v1/prices/history"
...
def prices_history(
    ...
    limit: int = Query(
        ...
        description=(
            "Max rows to return (260 ~ 5 years weekly). This endpoint applies no default "
            "date window; the applied_bounds block on the response reports the limit that "
            "was used and whether the result was truncated."
        ),
    ),
):
    """
    ...
    # row is dropped before the response is built.
    params = {
        "symbol": s,
        "exchange": ex,
